...
#                              Prompt templates                               #
# --------------------------------------------------------------------------- #
# Prompt text now lives in dvd.prompts.PROMPTS (caption_system / caption_prompt
# / merge_prompt) and is read at call time so overrides take effect.
...
def _caption_clip(task: Tuple[str, Dict], caption_ckpt_folder) -> Tuple[str, dict]:
    ...
    send_messages[0]["content"] = p.caption_system
    send_messages[1]["content"] = p.caption_prompt.replace(
...
def merge_subject_registries(registries: List[dict]) -> dict:
    ...
    send_messages = copy.deepcopy(messages)
    send_messages[0]["content"] = p.caption_system
    send_messages[1]["content"] = p.merge_prompt.replace(
